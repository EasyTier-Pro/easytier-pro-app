...
`.github/workflows/android.yml` 会在 Windows runner 上检出 EasyTier 固定 commit `c0f42ebe8c0b18e49370ae810b13fba1bdbbb811`，安装 Android NDK `28.2.13676358`，按 EasyTier 的 Rust toolchain 安装 `aarch64-linux-android` 和 `x86_64-linux-android` targets，执行 `dart analyze`、`flutter test`、`scripts/build_android_jni.ps1` 构建 JNI 并复制到临时工作区的 `android/app/src/main/jniLibs`，运行 `scripts/verify_android_release_inputs.ps1` 校验本次构建的 JNI ABI、release ABI filter、正式 `applicationId`、VPN manifest、cleartext 策略和签名文件忽略规则，构建 Android debug APK，并打包 `:app:assembleDebugAndroidTest` 以确保 JNI 基础集成测试可编译。CI 不要求真实 release keystore；更新仓库内随包 JNI 产物仍需在本地运行构建脚本并提交结果。CI 打包 androidTest APK 不等同于真机执行，设备侧仍需运行 `connectedDebugAndroidTest` 或手动真机验证。
