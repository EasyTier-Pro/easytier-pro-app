...
- debug 包连接中运行 `.\scripts\collect_android_e2e_evidence.ps1 -ExpectedRoute <虚拟网CIDR>,<子网CIDR> -RequireSystemRoute -PingTarget <虚拟IP或子网地址> -RequirePingSuccess -ProbePackageName <未排除应用包名> -RequireProbePackageRoute -RequireSelfExcludedRoute`，确认诊断日志、系统路由表、route probe、ping 成功证据和 `summary.json` 的 `passed=true` 均已采集。
...
- 退出登录后，debug 包运行 `.\scripts\collect_android_e2e_evidence.ps1 -ExpectedRoute <虚拟网CIDR>,<子网CIDR> -RequireStop -RequireConfigServerStop`，或对导出的诊断日志运行 `.\scripts\verify_android_e2e_diagnostics.ps1 -LogPath <diagnostics.log> -ExpectedRoute <虚拟网CIDR>,<子网CIDR> -RequireStop -RequireConfigServerStop`，确认最新一次 VPN 建立之后 config server client 与 VPN 均停止。
