...
- 退出登录后，debug 包运行 `.\scripts\collect_android_e2e_evidence.ps1 -ExpectedRoute <虚拟网CIDR>,<子网CIDR> -RequireStop -RequireConfigServerStop`，或对导出的诊断日志运行 `.\scripts\verify_android_e2e_diagnostics.ps1 -LogPath <diagnostics.log> -ExpectedRoute <虚拟网CIDR>,<子网CIDR> -RequireStop -RequireConfigServerStop`，确认 config server client 与 VPN 均停止。
