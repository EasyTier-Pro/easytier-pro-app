...
- `com.android.settings` 的默认排除只用于让开发者选项无线调试页继续展示物理网络地址；实际 ADB 传输依赖 `com.android.shell` 是否成功进入 `builder_disallowed_applications`。debug 构建还会设置 `allow_bypass`/`builder_allow_bypass=true`，允许 Flutter/ADB 这类显式绑定底层网络的调试链路绕过 VPN；release 构建不启用该调试逃逸能力。排查 VPN 数据面时不要用 `adb shell ping` 作为准绳，应继续使用 Termux、浏览器或其他未排除应用做 UID route probe。
...
- 应用内“设置 -> 诊断日志”出现 `Android VPN established`，且 `routes` 包含虚拟网 CIDR 与已授权子网 CIDR，`disallowed_applications` 至少包含 `net.easytier.pro`、`com.android.settings` 与 `com.android.shell`。
