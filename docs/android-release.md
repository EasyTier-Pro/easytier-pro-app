...
- 本地 debug 包连接中可以直接运行 `.\scripts\collect_android_e2e_evidence.ps1 -ExpectedRoute <虚拟网CIDR>,<子网CIDR> -RequireSystemRoute -PingTarget <虚拟IP或子网地址> -RequirePingSuccess -ProbePackageName com.termux`，脚本会通过 `adb run-as net.easytier.pro` 拉取应用日志，收集 `ip route`、`ip rule`、`ip addr`、`ip link`、`dumpsys connectivity`、`dumpsys netpolicy`、`dumpsys netd`、包信息、route probe 和 ping 输出，并调用诊断校验脚本；`route_probes.txt` 会对 `PingTarget` 和从 `ExpectedRoute` 推导出的目标运行普通 `ip route get`，并分别用 EasyTier Pro 自身 UID 与 `-ProbePackageName` 指定的未排除应用 UID 运行 `ip route get <target> uid <uid>`，前者用于确认自排除不会回环，后者用于判断诊断应用流量是否会进入 VPN 规则/接口。系统 route table 检查会归一化 `-ExpectedRoute`。`-RequireSystemRoute` 必须配至少一个 `-ExpectedRoute`，`-RequirePingSuccess` 必须配至少一个 `-PingTarget`，避免空目标误判通过。如果只是排查失败现场，可以临时去掉 `-RequireSystemRoute` 或 `-RequirePingSuccess` 以保留失败证据；发布前数据面验收必须保留这两个开关。release 包无法使用 `run-as`，应改用应用内分享面板导出诊断日志。
...
- debug 包连接中运行 `.\scripts\collect_android_e2e_evidence.ps1 -ExpectedRoute <虚拟网CIDR>,<子网CIDR> -RequireSystemRoute -PingTarget <虚拟IP或子网地址> -RequirePingSuccess -ProbePackageName <未排除应用包名>`，确认诊断日志、系统路由表、route probe 和 ping 成功证据均已采集。
