...
```text
75cd6ab4c6809875b76f18efd00e8cf091dd3f52
EasyTier/EasyTier#2326 Android JSON RPC / instance list / config server callback 扩展
```
...
上游 PR #2326 提供 `listInstances(maxLength)` 和 `callJsonRpc(serviceName, methodName, domainName, payloadJson)`。当前 Android runtime 使用 `listInstances` 定位 EasyTier instance name / instance id，并通过 JSON RPC 读取运行态：
...
JNI config server client 和 JSON RPC 能力非常新，应固定到明确 commit。当前构建固定到 EasyTier/EasyTier#2326 的 `75cd6ab4c6809875b76f18efd00e8cf091dd3f52`，不再依赖本仓库本地 patch。
